...
public class MessageManager {
    ...
    public MessageManager(XDiscordUltimate plugin) {
        this.plugin = plugin;
        this.prefix = colorize("&8[&bXDiscord&8] ");
        loadMessages();
    }
    ...
    public void sendActionBar(Player player, String message) {
        try {
            // Modern Adventure library support (Paper/Spigot 1.16.5+)
            ...
            // Fallback for older versions using Spigot's method
            try {
                ...
                player.spigot().getClass().getMethod("sendMessage", chatMessageTypeClass, baseComponentClass)
                    .invoke(player.spigot(), actionBar, components[0]);
            } catch (Exception ex) {
                // Final fallback: just send as a regular message
