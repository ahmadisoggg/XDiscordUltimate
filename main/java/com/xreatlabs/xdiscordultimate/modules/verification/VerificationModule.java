        ...
        plugin.getDatabaseManager().linkAccount(
            ...
            if (success) {
                ...
                Bukkit.getScheduler().runTask(plugin, () -> {
                    ...
                    // Trigger sync
                    com.xreatlabs.xdiscordultimate.modules.sync.SyncModule syncModule = plugin.getModuleManager().getModule(com.xreatlabs.xdiscordultimate.modules.sync.SyncModule.class);
                    if (syncModule != null && syncModule.isEnabled()) {
                        Member member = plugin.getDiscordManager().getMainGuild().getMemberById(pending.discordId);
                        if (member != null) {
                            syncModule.syncRank(player, member);
                            syncModule.syncNickname(player, member);
                        }
                    }
                });
    ...
    private void notifyDiscordVerification(String discordId, String minecraftName) {
        ...
        try {
            ...
            if (user != null) {
                ...
                warning("Could not find user with ID " + discordId + " to send verification DM.");
            }
        } catch (Exception e) {
