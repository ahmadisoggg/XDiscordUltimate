...
public class ChatBridgeModule extends Module implements Listener {
    ...
    private void loadConfiguration() {
        ...
        if (chatChannelName == null || chatChannelName.isEmpty()) {
            chatChannelName = getConfig().getString("chat-channel");
        }

        if (chatChannelName == null || chatChannelName.isEmpty()) {
            chatChannelName = "minecraft-chat";
            warning("ChatBridge: 'chat-channel-id' or 'chat-channel' not found in config, defaulting to 'minecraft-chat'.");
        }
    ...
    private TextChannel getChatChannel() {
        ...
        // As a secondary check, look for a globally defined chat channel ID.
        String globalChannelId = plugin.getConfig().getString("discord.channels.chat");
        if (globalChannelId != null && !globalChannelId.isEmpty() && !globalChannelId.equals("YOUR_CHAT_CHANNEL_ID")) {
            TextChannel channel = plugin.getDiscordManager().getTextChannelById(globalChannelId);
            if (channel != null) {
                return channel;
            }
        }
        ...
        return plugin.getDiscordManager().getMainGuild()
            ...
            .findFirst()
            .orElseGet(() -> {
                debug("ChatBridge: Could not find channel with ID or name: " + chatChannelName);
                return null;
            });
    }
